"""
Person 1's core deliverable: WhatsApp voice-note in -> transcript -> (hook to
teammates' NLU/recommendation logic) -> voice-note out.

Flow for one incoming message:
  1. Twilio POSTs to /webhook when a WhatsApp voice note arrives
  2. We download the audio Twilio give us a URL to
  3. We send that audio to Sarvam's Speech-to-Text API -> get transcript
  4. We hand the transcript to get_recommendation_reply() -- THIS IS WHERE
     PERSON 2's NLU + recommendation engine plugs in. Right now it's a stub.
  5. We send the reply text to Sarvam's Text-to-Speech API -> get audio back
  6. We save that audio locally and serve it via Flask's /static route
  7. We reply to Twilio with TwiML pointing at that audio file's public URL
     (ngrok makes your local server reachable, so Twilio can fetch it)
"""
